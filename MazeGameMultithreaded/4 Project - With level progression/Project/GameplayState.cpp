...
constexpr double msBetweenDraw = 33;
constexpr double msBetweenMove = 1000;
...
void GameplayState::Enter()
{
	Load();
	endThreads = false;
	GameplayThread = std::thread(&GameplayState::ActorUpdate_Threaded, this);
	DrawThread = std::thread(&GameplayState::Draw_Threaded, this);
}
...
void GameplayState::HandleCollision(int newPlayerX, int newPlayerY)
{
	...
	{
		//Do not move. Need to rehandle collision in the original spot.
		HandleCollision(m_player.GetXPosition(), m_player.GetYPosition());
	}
}

void GameplayState::ActorUpdate_Threaded() 
{
	double moveFrame = 0;
	chrono::time_point<chrono::high_resolution_clock> now;
	chrono::time_point<chrono::high_resolution_clock> lastUpdate = std::chrono::high_resolution_clock::now();
	double dt;
	while (!endThreads)
	{
		now = std::chrono::high_resolution_clock::now();
		dt = std::chrono::duration<double, std::milli>(now-lastUpdate).count();
		lastUpdate = now;
		...
		if (moveFrame > msBetweenMove)
		{
			...
			moveFrame = fmod(moveFrame, msBetweenMove);
		}

	}
}

void GameplayState::Draw_Threaded()
{
	double drawFrame = 0;
	chrono::time_point<chrono::high_resolution_clock> now;
	chrono::time_point<chrono::high_resolution_clock> lastUpdate = std::chrono::high_resolution_clock::now();
	...
	while (!endThreads)
	{
		...
		if (drawFrame > msBetweenDraw)
		{
			...
			drawFrame = fmod(drawFrame, msBetweenDraw);
		}
	}
}
